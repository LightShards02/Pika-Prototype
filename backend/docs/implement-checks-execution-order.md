...
27. `[v0.0.5][agent+deterministic]` **Run Reviewer Loop** (`implement.reviewer.enabled`): After batch execution, run the bounded review loop. Per iteration: collect deterministic harness results + verification evidence, fan out one **per-spec reviewer** LLM call per spec_id in the batch (parallel via thread pool, bounded by `reviewer.per_spec_max_total_seconds`), then run a deterministic Python **synthesis pass** that aggregates per-spec axis findings, applies the axis-3/4 demotion rule, decides `response_kind`, and emits a structured `amendment_packet` with stable `amendment_id`s of the form `{spec_id}::{criterion_id}::{axis}`.
    a. Per-spec reviewer reviews four axes for one spec at a time: code↔AC, test_plan↔AC, test_code↔test_plan, test_run_evidence. Verification timeouts surface as `insufficient_evidence` to axis 4 — never `pass`.
    b. Synthesis (Python) produces the final `reviewer_output` discriminated by `response_kind`: `approve` / `amend` / `manual_block`. Demoted axis 3/4 findings (when axis 1 passes for the same spec) flow into top-level `minor_findings[]` instead of driving an amendment.
    c. On `approve` → exit loop, lifecycle continues to translate.
    d. On `amend` → re-invoke the implementer in `amendment_only` mode (`implement_from_specs_amend` / `_local_amend`) with the structured packet, prior `diff_plan`, and prior `authored_test_cases`. Iteration advances. The merge in `_execute_batch` combines prior diffs + amendment delta so the per-iteration worktree applies cumulative state.
    e. On `manual_block` → escalate immediately with the reviewer's `kind` (`ambiguity` or `scope_conflict`).
    f. **Stagnation check** (deterministic): when iteration N's amendment_id set fully contains iteration N-1's, escalate as `amendment_unsatisfiable`.
    g. **Partial-spec lock-in** (deterministic): when iteration N's `diff_plan` touches a non-target spec_id (and that spec wasn't a prior shared-infrastructure owner), revert and escalate as `amendment_unsatisfiable`.
    h. **Loop limit**: when `iteration == reviewer.max_iterations` without `approve`, escalate as `loop_limit_exceeded`.
    i. Per-spec call wall-clock budget exhausted → batch fails with `reviewer_per_spec_timeout`.
    j. Skipped when `reviewer.enabled=false` or in dry-run mode.
    k. Produces: `agent_outputs/reviewer_output_{batch_id}_iter{n}.json`, `agent_outputs/reviewer_output_{batch_id}.json` (latest pointer), `agent_outputs/reviewer_harness_{batch_id}_iter{n}.json`, `agent_outputs/review_loop_{batch_id}.json` (per-iteration log for resume), `agent_outputs/implement_{batch_id}_iter{n}.json` for amendment-only iterations, and (on block) `manual_resolution/reviewer_block_{batch_id}.json` / `manual_resolution/reviewer_unsat_{batch_id}.json` / `manual_resolution/reviewer_loop_limit_{batch_id}.json`.

    > **Verification gating note:** When `reviewer.enabled=true` and `reviewer.demote_verification_to_evidence=true` (default), step 26 (`verification_execution`) demotes from a hard gate to evidence. Each verification command runs with a per-command timeout (`implement.verification.timeout_seconds`, default 300s); exit codes and timeouts are captured into `verification_evidence_{batch_id}.json` and surfaced to the reviewer's axis 4 instead of failing the batch. When `reviewer.enabled=false`, step 26 retains today's hard-gate behavior (verification failure or timeout fails the batch).

    > **Legacy code evaluator (deprecated, scheduled for next-release removal):** when `reviewer.enabled=false` AND `implement.evaluator.enabled=true`, the legacy `code_evaluator` path runs in place of the reviewer loop. It produces `agent_outputs/code_eval_cycle_{N}.json` + `agent_outputs/harness_results_cycle_{N}.json` and on block `manual_resolution/code_evaluation.json`. New deployments should set `reviewer.enabled=true`. The evaluator emits a deprecation warning at runtime when invoked.
