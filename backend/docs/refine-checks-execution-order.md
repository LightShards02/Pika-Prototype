...
7. `[v0.0.0][agent]` **Run Ambiguity Detector Agent** (`spec_ambiguity_detector`): Invoke ambiguity detector agent with schema retry loop.
  a. Build prompt variables: `project_context`, `design_spec_csv`, `manual_resolution_file`, `run_summary_file`, `output_schema_file`.
   b. Send to the ambiguity detector agent via `invoke_agent_with_schema_retry`.
   c. Parse returned JSON against the `spec_ambiguity_detector_output` schema.
   d. Retry agent generation if schema validation fails.
   e. Runs in parallel with step 8.
   f. Produces: `ambiguity_output.json`.
8. `[v0.0.0][agent]` **Run Testability Auditor Agent** (`spec_testability_auditor`): Invoke testability auditor agent with schema retry loop.
  a. Build prompt variables: `project_context`, `design_spec_csv`, `manual_resolution_file`, `run_summary_file`, `output_schema_file`.
   b. Send to the testability auditor agent via `invoke_agent_with_schema_retry`.
   c. Parse returned JSON against the `spec_testability_auditor_output` schema.
   d. Retry agent generation if schema validation fails.
   e. Runs in parallel with step 7.
   f. Produces: `testability_output.json`.
  > **Parallelism note:** Steps 7 and 8 execute concurrently via `ThreadPoolExecutor(max_workers=2)`. Both must complete (or fail) before the pipeline continues. If either agent fails, the entire refine run fails with the error detail.
9. `[v0.0.0][deterministic]` **Merge Manual Resolution Items** (`refine.item_merge`): Combine `manual_resolution_items` from all sources in order.
  a. Collect items from ambiguity detector output (`manual_resolution_items` key).
   b. Collect items from testability auditor output (`manual_resolution_items` key).
