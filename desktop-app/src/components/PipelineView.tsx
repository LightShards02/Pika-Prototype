...
export const PipelineView = () => {
  const { phases, refineEnabled, implementEnabled, decompositionEnabled } = useStore();

  const refinePhases = phases.filter(
    p => p.group === 'Refine' && (p.id !== 'R2' || decompositionEnabled)
  );
            ...
            {refinePhases.map((phase) => (
              <div
                ...
                className={clsx(
                  ...
                  phase.status === 'running' ? "bg-bg-elevated border-border-subtle shadow-sm" : "hover:bg-bg-panel"
                )}
              >
