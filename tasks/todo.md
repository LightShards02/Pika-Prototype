...
- [x] M6.5: workspace phase-run listing endpoint `GET /v1/workspaces/{id}/phase-runs` (phase/status/chain_id filters, started_at desc, default limit 100, max 500). Merged as `ffeaba1`. 14 new listing tests (965 total). 2 review rounds: round 1 flagged malformed-`started_at` lexicographic leapfrog (Tier 1, e.g., `"zzz"` could sort before valid timestamps under naive `reverse=True`); round 2 accepted after regex-validated two-bucket sort (`_STARTED_AT_RE`) + `test_malformed_started_at_sorts_last` regression. Non-blocking centralize-regex suggestion deferred.
...
Per user direction: `plan.design`, `resolve_plan.organize`, `review.audit`, and the legacy `/resolve` REST surface are explicitly out of scope. Remaining milestones: M5 login OAuth (small, skipped for now), M7-M8 chat orchestrator (router + curator), M9 desktop cutover, M10 hardening + OpenAPI + compaction.
