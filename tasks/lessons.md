...
- Pattern: When validating refine across many scenario sub-datasets in parallel, full-mode `spec_quality_auditor` LLM calls (long-running, high token output) had their HTTP streams dropped mid-response with "peer closed connection without sending complete message body" errors. Failure manifested when 4+ scenarios fanned out simultaneously (each spinning N replicas → 16+ concurrent streaming connections through the local Loca/codex pipeline). Triage replicas (short, fast) were unaffected; only full-mode calls.
  Rule: When orchestrating refine validation across multiple scenario directories, cap **concurrent full-mode calls at 2** at any moment (i.e. no more than 2 scenarios in flight in parallel). Triage replicas inside one scenario can still run wide because they are short. This is a runtime concurrency constraint for the validation harness, not a PIKA code bug — the pipeline is correct; only the launch pattern needs throttling.

- Pattern: When converting an external dataset (e.g. CSI/TimeLapse traceability matrix) into PIKA's `raw-design-spec.csv` format, the user constrained changes to filename + column-rename + column-drop only — no row-content edits. Defaulting to richer transformations (re-deriving `module_role` from subcategory text, normalizing values, etc.) would have violated the constraint.
  **Why:** dataset content under regulatory authoring conventions is owned upstream; PIKA's job is normalization to its schema, not editorial rewriting. Content-side fixes belong to the upstream document workflow.
  **How to apply:** for any "convert <external> for refine/format" request, first ask whether row-content edits are allowed. If the user hasn't explicitly authorized them, default to schema-only operations: rename CSV columns, drop columns, rename file. Let `format` and `refine` do the rest via their existing pipelines (spec_id auto-assignment, module_tag derivation, enrichment for module_role).

- Pattern: The `quality_item.options` array in the spec_quality_auditor schema looks like dead boilerplate (always the same 3-tuple) but is load-bearing — `option_id` flows into `core/resolution.py:101-111` (manual_edit injection) and `handlers/resolve.py:325-326` (`issue_kind = "field" if "accept_suggestion" in option_ids else "structural"`, gating spec_editor invocation mode).
  **Why:** removing it from the agent output schema saves tokens, but consumers that index into `option_id` are silent failures if the field disappears. Same trap with `field` (used by terminal renderer in `resolve.py:112-114` and the spec_editor template variable).
  **How to apply:** when proposing schema slimming for an agent output, always run a consumer audit (Explore agent or grep) before deciding the field is safe to delete. If consumers exist, move the constant injection to the v3->v2 translation layer (e.g. `_translate_v3_item_to_v2_legacy`) — the agent stops emitting, the translator injects the canonical literal, and downstream code sees no break. This pattern also eliminates a class of schema-validation flakes from per-replica regeneration of constant text.

- Pattern: UI validation (Playwright MCP, browser tools, dogfood flows) leaves behind screenshot files that clutter the workspace and are easy to forget.
  Rule: After every UI validation pass, delete all screenshots that were created for that validation (project temp dirs, browser-logs paths, or any explicit capture paths used during the run). Treat cleanup as part of "done," not optional housekeeping.
